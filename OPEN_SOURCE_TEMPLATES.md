...
The complete upstream sources are vendored under `vendor/open-source-templates/<id>`, including the upstream license files **and their built `dist/` output**. The `dist/` directories are required at runtime — `lib/template-static.ts` serves `<template>/dist/index.html` — so they are committed alongside the sources; a plain `git clone` is enough to render every template, and no submodule init is needed. The templates' own `.gitignore` files have their `dist/` rule commented out for this reason (see the root `.gitignore`). RicoFast declares MIT in its `package.json`. The application renders allowlisted official demos through a read-only preview adapter and maps structured content into those pages without executing upstream scripts.
